...
export class AgendamentosComponent implements OnInit {
  ...
  statusForm = new FormGroup({
    paymentMethdod: new FormControl({value: '', disabled: true},[
      Validators.required
    ]),
        ...
        for (let schedule of schedules)  {
            if(schedule['status'] === status) {
              const row = this.isProfessional() ? [this.formatDate(schedule['date']), schedule['patient'], schedule['services']] 
                          : [this.formatDate(schedule['date']), schedule['patient'], schedule['professional'], schedule['finalPrice'] + ' R$', schedule['services']];
              rows.push(row);

              const tail = [schedule['_id'], schedule['paymentMethdod'], schedule['paymentMade'], schedule['status'], schedule['patient'], schedule['clinicalStory'], schedule['priorityQueue']];
              tails.push(tail);
            }
        }
  ...
  public setNavActive(status) {
    ...
    if (status === 'Aguardando') $('#status1').addClass('active');
    else if (status === 'Atendido') $('#status2').addClass('active');
    else $('#status3').addClass('active');
    this.showLoad = false;
  }
  ...
  setPageUsers(pageNumber: any) {
    ...
    this.tableData1 = {
      headerRow: this.isProfessional() ? ['Data', 'Paciente', 'Serviços'] :['Data', 'Paciente','Profissional' ,'Valor Final', 'Serviços'],
  ...
  openModal(index) {
    ...
    this.statusForm.get('paymentMethdod').setValue(schedulingDetails[1]);
    this.statusForm.get('paymentMade').setValue(schedulingDetails[2]);
    this.statusForm.get('status').setValue(schedulingDetails[3]);
    $('#myModal').appendTo("body").modal('show');
  }

  getPaymentOption() {
    if(this.statusForm.get('paymentMethdod')) {
      return this.statusForm.get('paymentMethdod').value;
    } else {
      return ''
    }
  }
    ...
    request.subscribe(
      (response) => {
        for(let key of SINGALS_KEYS) {
          ...
          if(disabled) this.signalsForm.get(key).disable();
        }
      }, (err) => {
        console.log(err);
      }
